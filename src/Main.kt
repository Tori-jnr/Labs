...
//   println(houseNumbers) it just prints the array's memory reference not the actual values stored inside it.
   ...
   //but a read-only List has no methods to modify its contents, regardless of val or var.
    ...
    // var tenantEmail: String = null    error: null cannot be a value of a non-null 'String'
    //add the character '?'
...
//Assumption var tenantEmail: String? = null
    ...
    //println(tenantEmail!!.length) would CRASH here with NullPointerException since tenantEmail is null right now

    tenantEmail = "jane@example.com"
    println(tenantEmail!!.length)//output will be the length of the email since it now exists
